#!/usr/bin/env node
/**
 * Assert critical project files exist (single-tab SPA; no compare.html).
 */
...
// Vendor TradingView Lightweight Charts standalone ESM into public/ (Worker static assets)
...
if (existsSync(join(root, "public/compare.html"))) {
  console.error("build FAIL: public/compare.html must not exist (tabs are hash-based SPA)");
  process.exit(1);
}

const wrangler = readFileSync(join(root, "wrangler.jsonc"), "utf8");
if (!wrangler.includes("fx-analysis.tanishqnalloju.com")) {
  console.error("build FAIL: wrangler.jsonc must route fx-analysis.tanishqnalloju.com");
  process.exit(1);
}
...
if (/"\/compare"/.test(wrangler) || wrangler.includes("/compare/*")) {
  console.error("build FAIL: run_worker_first must be /api/* only (no /compare routes)");
  process.exit(1);
}

const indexHtml = readFileSync(join(root, "public/index.html"), "utf8");
if (!indexHtml.includes("data-tab") || !indexHtml.includes("FX Analysis")) {
  console.error("build FAIL: index.html must be tabbed FX Analysis shell");
  process.exit(1);
}
...
if (!indexHtml.includes("asOfStrip") || !indexHtml.includes("methodGrid") || !indexHtml.includes("reerPanel")) {
  console.error("build FAIL: index.html missing as-of strip / methodology / REER panel");
  process.exit(1);
}
...
if (!indexHtml.includes("cmpTechStrip") || !indexHtml.includes("cmpModelTable") || !indexHtml.includes("cmpLeadTable")) {
  console.error("build FAIL: index.html missing compare technical/model/lead-lag panels");
  process.exit(1);
}
...
if (!required.includes("scripts/seed-kv.mjs")) {
  /* seed-kv checked below */
}
...
console.log("  ✓ no compare.html · wrangler → fx-analysis.tanishqnalloju.com");
